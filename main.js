...
const mostRequestedImages = [
  { src: './img_proy/polygel/polygel-1.jpeg', name: 'Natural elegante', category: 'polygel', note: 'Look limpio y delicado' },
  { src: './img_proy/acrilicas/acrilicas-23.jpeg', name: 'Acrílicas con estilo', category: 'acrilicas', note: 'Para manos protagonistas' },
  { src: './img_proy/polygel/polygel-40.jpeg', name: 'Brillo glam', category: 'polygel', note: 'Ideal para ocasiones especiales' },
  { src: './img_proy/semipermanente/semipermanente-1.jpeg', name: 'Semipermanente bonito', category: 'semipermanente', note: 'Práctico y femenino' },
  { src: './img_proy/builder_gel/builder_gel-2.jpeg', name: 'Uña reforzada', category: 'builder-gel', note: 'Para uñas delicadas' },
  { src: './img_proy/acrilicas/acrilicas-13.jpeg', name: 'Diseño delicado', category: 'acrilicas', note: 'Elegante para diario' },
];
...
function createGalleryItem(item, globalIndex, animDelay = 0) {
  ...
  const waMessage = encodeURIComponent(
    `Hola Angie Nails Studio! Quiero este diseño: ${item.name}. ¿Me puedes decir el precio, duración y horarios disponibles?`
  );
...
function createMostRequestedItem(item, index) {
  ...
  const waMessage = encodeURIComponent(
    `Hola Angie Nails Studio! Quiero este diseño más pedido: ${item.name}. ¿Me puedes decir el precio, duración y horarios disponibles?`
  );
  const waUrl = `https://wa.me/51936926560?text=${waMessage}`;
  ...
  div.innerHTML = `
    ...
    <div class="requested-card-content">
      ...
      <h4>${item.name}</h4>
      <p>${item.note}</p>
      <a href="${waUrl}" target="_blank" class="requested-card-cta" onclick="event.stopPropagation()">Quiero este diseño</a>
    </div>
  `;
...
function renderMostRequested() {
  ...
  if (!grid) return;
  grid.innerHTML = "";
  mostRequestedImages.forEach((item, index) => {
    grid.appendChild(createMostRequestedItem(item, index));
  });
}
